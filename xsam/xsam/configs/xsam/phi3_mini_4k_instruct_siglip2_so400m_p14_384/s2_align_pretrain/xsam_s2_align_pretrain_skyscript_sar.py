...
# Specify the pretrained pth - 使用第一阶段训练的权重
s1_pretrained_pth = root_dir + "checkpoints/s1_seg_finetune/pytorch_model.bin"

# Data - SkyScript数据集（75w条数据）
skyscript_data_path = data_dir + "img_conv_data/skyscript/skyscript.json"
# SkyScript数据集的图片路径格式：images2/xxx.jpg，所以image_folder应该是datasets根目录
...
SYSTEM = ""
# 使用 root_dir 构建绝对路径，避免依赖 work_dir 中的文件复制
evaluation_images = root_dir + "xsam/xsam/configs/xsam/images/imgconv.png"
...
# SAR训练数据集配置
sar_train_imgconv_dataset = dict(
    type=ImageConvDataset,
    data_path=sar_train_data_path,
    tokenizer=tokenizer,
    image_folder=sar_image_folder,
    task_name="imgconv",
    data_name="sar_train_imgconv",
    image_processor=image_processor,
    extra_image_processor=extra_image_processor,
    dataset_map_fn=image_conv_map_fn,
    template_map_fn=dict(type=template_map_fn_factory, template=prompt_template),
...
# 组合训练数据集：SkyScript + SAR训练集
combined_train_dataset = dict(
    type=ConcatDataset,
    oversample_ratio=0.0,  # 不使用自动平衡，保持原始数据比例
    datasets=[
        skyscript_imgconv_dataset,
        sar_train_imgconv_dataset,
    ],
)
