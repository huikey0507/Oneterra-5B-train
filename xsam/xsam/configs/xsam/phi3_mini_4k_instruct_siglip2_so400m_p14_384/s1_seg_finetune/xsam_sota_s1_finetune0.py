...
# Data - 修改为SOTA数据集路径
data_root = data_dir + "sota/"
# 训练集路径
train_data_path = data_root + "train_annotations.json"  # 训练集标注文件
train_image_folder = data_root + "train/images"  # 训练集图像文件夹
train_panseg_map_folder = data_root + "train/panoptic_labels"  # 训练集RGB标签文件夹
    ...
    segmentor=dict(
        ...
        decoder=dict(
            ...
            config=dict(
                ...
                num_labels=39,  # SOTA数据集：39个类别
                trust_remote_code=True,
            ),
            torch_dtype=torch.bfloat16,
        ),
        torch_dtype=torch.bfloat16,
        reinit_decoder=False,  # 确保不重新初始化整个解码器，复用预训练权重
        close_cls=True,  # 确保类别预测层可以独立初始化
    ),
)
...
sota_panoptic_genseg_dataset = dict(
    ...
    data_path=train_data_path,  # 训练集标注文件
    image_folder=train_image_folder,
    panseg_map_folder=train_panseg_map_folder,
    extra_image_processor=train_extra_image_processor,
    task_name="genseg",
    data_name="sota_panoptic_genseg_train",  # 训练集名称
...
train_dataloader = dict(
    ...
    pin_memory=True,
    dataset=sota_panoptic_genseg_dataset,
    sampler=dict(
