...
dataloader_num_workers = 0  # 设为0以节省内存（主进程加载数据，无额外worker进程）
max_epochs = 2
optim_type = AdamW
lr = 4e-5
betas = (0.9, 0.999)
...
SYSTEM = ""
# 设置为 None 以禁用训练过程中的评估，或使用自定义验证数据集
evaluation_images = None  # 如果使用自定义验证，可以在这里设置自定义图片路径列表
evaluation_inputs = [
...
# 1. Generic Segmentation (genseg) - SOTA data
...
# 2. Open-Vocabulary Segmentation (ovseg) - SOTA data
...
# 3. Referring Segmentation (refseg) - RemoteSAM data
...
# Combine three training datasets
combined_train_dataset = dict(
    type=ConcatDataset,
    datasets=[sota_genseg_dataset, sota_ovseg_dataset, remotesam_refseg_dataset],
)
...
val_datasets = [
    ...
    dict(
        type=GenericSegDataset,
        data_path=data_dir + "sota/val_annotations.json",
        image_folder=data_dir + "sota/val/images",
        panseg_map_folder=data_dir + "sota/val/panoptic_labels",
        data_mode="eval",
        tokenizer=tokenizer,
        task_name="genseg",
...
custom_hooks = [
    ...
    # 禁用 EvaluateChatHook：当 evaluation_images=None 时，它只做纯文本评估（没有图像），输出无意义
    # 真正的验证应该在训练完成后使用 xsam/xsam/tools/eval.py 脚本进行
    # 验证数据集配置在 val_datasets 中（使用自定义数据集：SOTA和RemoteSAM）
    # 如果需要启用快速检查，需要提供 evaluation_images 列表（固定评估图片路径）
    # dict(
    ...
    #     evaluation_images=evaluation_images,  # 需要提供图片路径列表，不能为None
