...
model = dict(
    ...
    segmentor=dict(
        ...
        torch_dtype=torch.bfloat16,
        reinit_decoder=False,  # 续训：保留已学 decoder
        open_cls=True,
    ),
)
...
pano_ovseg_dataset = dict(
    ...
    use_variant_cat=True, use_full_cat=True, sample_num=32, variant_subset_prob=0.7,
    use_cat_synonym=True, cat_synonym_keep_prob=0.5,
    repeats_scale=4.0,
)  # 同义词 + 70% 子集开集；略提权（3.0→4.0）
...
# 从 v31 / s3.1_134072 续训（不 resume 优化器状态）
