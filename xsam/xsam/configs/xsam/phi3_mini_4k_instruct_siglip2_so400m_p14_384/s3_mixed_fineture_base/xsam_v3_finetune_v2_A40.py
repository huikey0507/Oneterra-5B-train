...
# 独立的 V4 工作目录，防止覆盖 V1 断点
work_dir = getenv("WORK_DIR", base_root + "shui/LAE/OneTerra-train/checkpoints/xsam_v3_finetune_A40")
...
dataloader_num_workers = 0  # ✅ 已设置为 0，避免多进程内存重复
max_epochs = 2
optim_type = AdamW
lr = 2e-5                
betas = (0.9, 0.999)
...
pano_ovseg_dataset = dict(
    ...
    # 🚀 极其关键：修复 OVSeg 动态采样以激活开集能力
    use_variant_cat=True, use_full_cat=True, sample_num=32, 
    repeats_scale=2, # 22万次重点曝光抢救 PQ
)
...
rsvqa_lr_imgconv_dataset = dict(
    type=ImgConvDataset, 
    data_path=oneterra_data_root + "imgconv/VQA/RSVQA-LR/train_cleaned.json", # 指向原始全量文件
    ...
    repeats_scale=4, # 提权 4 倍，强化 VQA 答题格式
)
...
risbench_refseg_dataset = dict(
    type=RefSegDataset, 
    data_root=oneterra_data_root + "refseg/RISBench",  # ✅ 修复：去掉末尾的 /risbench，让 REFER 类自动拼接
    ...
    repeats_scale=1, sample_num=8,  # 5.2万条，限制 mask 数量防止 OOM
)

rrsisd_refseg_dataset = dict(
    type=RefSegDataset, 
    data_root=oneterra_data_root + "refseg/RRSIS-D",  # RRSIS-D 数据集根目录
    ...
    template_map_fn=dict(type=template_map_fn_factory, template=prompt_template), 
    use_variant_cat=True, use_random_cat=True, max_length=max_length, pad_image_to_square=False, ignore_label=ignore_label, 
    repeats_scale=1, sample_num=8,  # 约1.7万条，限制 mask 数量防止 OOM
)
...
combined_train_dataset = dict(
    ...
    datasets=[
        ...
        diy1_reaseg_dataset,
        risbench_refseg_dataset,  # ⚠️ 临时注释：测试是否导致 OOM
        rrsisd_refseg_dataset,  # ⚠️ 临时注释：测试是否导致 OOM
