"""Joint Stage B∥B2 + SARDET: train connector & cond_adapter together.

Skip standalone Stage B. One shot from S3 + Stage A:
  ① S3 full optical base (s2_pretrained_pth)
  ② Stage A SAR projectors + router (sar_adapter_pth)
  ③ sar_seg_connector starts from optical→SAR copy (then trained)
  ④ sar_cond_adapter zero-init residual (trained jointly)

Train: sar_seg_connector + sar_cond_adapter (+ modality_router)
Freeze: LLM/SigLIP/SAM encoder, Mask2Former decoder,
        optical adapters, SAR projectors (Stage A)

Data mix (effective ≈ raw * repeats_scale)
-----------------------------------------
- ParSAR   3642  * 10.0 ≈ 36420  (stuff+ship anchor)
- SARDET  94493  * 0.35 ≈ 33072  (6 things; downsampled)
- Optical 111066 * 0.15 ≈ 16660  (anti-forget + synonym)

Approx ParSAR:SARDET:OPT ≈ 2.2 : 2.0 : 1.0
Expected iters @4 GPU ≈ (36420+33072+16660)/4 ≈ 21500
"""
...
work_dir = getenv(
    "WORK_DIR",
    base_root + "shui/LAE/OneTerra-train/wkdrs_sar_align_B2_joint_sardet",
)
...
# Joint B∥B2: S3 + A only. Do NOT load Stage B / B2_v2 (retrain connector+cond).
...
# ParSAR — ONLY SAR stuff source (sea/land). Keep high weight.
...
# SARDET — thing-only multi-class expansion
