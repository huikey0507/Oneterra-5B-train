"""Stage B2+SARDET: continue from B2_cond_v2, mix ParSAR + SARDET + optical pano.

Goal
----
- Keep ParSAR test performance (esp. stuff: sea/land)
- Add SARDET multi-class thing open-set ability
- Enable category synonyms for SAR prompts

Data mix (effective ≈ raw * repeats_scale)
-----------------------------------------
- ParSAR   3642  * 10.0 = 36420  (stuff+ship anchor; slightly higher than B2's 8x)
- SARDET  94493  * 0.35 = 33072  (6 things only; downsampled so it cannot drown stuff)
- Optical 111066 * 0.15 = 16660  (anti-forget optical OV + synonym)

Approx ratio ParSAR:SARDET:OPT ≈ 2.2 : 2.0 : 1.0
Expected iters @4 GPU ≈ (36420+33072+16660)/4 ≈ 21500 (~1.3–1.5x B2 walltime)

Init: load B2_cond_v2/pytorch_model.bin as s2, sar_adapter_pth=None
      → keep already-trained SAR connector/cond_adapter (no optical→SAR wipe).
"""
...
work_dir = getenv(
    "WORK_DIR",
    base_root + "shui/LAE/OneTerra-train/wkdrs_sar_align_B2_cond_v3_sardet",
)
...
# Continue from finished B2_cond_v2 (full SAR stack already inside this bin).
# Do NOT pass Stage A/B adapter list again — that would optical→SAR wipe then re-overlay.
s3_pretrained_pth = getenv(
    "PREV_B2_CKPT",
    base_root + "shui/LAE/OneTerra-train/wkdrs_sar_align_B2_cond_v2/pytorch_model.bin",
)
...
max_epochs = 1
optim_type = AdamW
# Slightly lower LR than B2 (5e-4): preserve ParSAR while absorbing SARDET
lr = 2e-4
betas = (0.9, 0.999)
weight_decay = 0.05
max_norm = 1
...
sardet_root = base_root + "shui/LAE/detection2mask_tool/output/"
...
# Optical OV-seg with synonyms — anti-forgetting + open-vocab replay
...
# ParSAR — ONLY source of SAR stuff (sea/land). Keep high weight.
sar_parsar_ovseg_train = dict(
    ...
    tokenizer=tokenizer,
    task_name="ovseg",
    # OVSegDataset requires "pano_ovseg" or "panoptic" in data_name
    data_name="sar_parsar_pano_ovseg_train",
    modality=1,
    cond_type=cond_type,
    special_tokens=special_tokens,
    image_processor=image_processor,
    ...
    use_cat_synonym=True,
    cat_synonym_keep_prob=0.5,
    # 3642 * 10 ≈ 36420; protect stuff against SARDET flood
    repeats_scale=10.0,
)

# SARDET (SAM-converted detection→panoptic) — thing-only multi-class expansion
sar_sardet_ovseg_train = dict(
    type=SarOVSegDataset,
    data_path=sardet_root + "annotations/panoptic_train.json",
    image_folder=sardet_root + "train",
    panseg_map_folder=sardet_root + "annotations/panoptic_train",
    tokenizer=tokenizer,
    task_name="ovseg",
    data_name="sar_sardet_pano_ovseg_train",
    modality=1,
    cond_type=cond_type,
    special_tokens=special_tokens,
    image_processor=image_processor,
    ...
    use_cat_synonym=True,
    cat_synonym_keep_prob=0.5,
    # 94493 * 0.35 ≈ 33072; comparable to ParSAR, not dominant
    repeats_scale=0.35,
)
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        # Order: ParSAR first (stuff anchor), then SARDET, then optical
        datasets=[sar_parsar_ovseg_train, sar_sardet_ovseg_train, opt_ovseg_train],
    ),
    sampler=dict(
