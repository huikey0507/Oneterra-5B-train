...
from xsam.dataset.map_fns.dataset_map_fns import image_conv_map_fn, generic_seg_map_fn, ov_seg_map_fn, reason_seg_map_fn, refer_seg_map_fn
...
reasonseg_data_root = data_dir + "reason_seg_data/"
...
# 5. Reasoning Segmentation (reasonseg) - LISA data
# 注意：需要根据实际数据路径调整 data_root, image_folder, explain_path
lisa_reasonseg_dataset = dict(
    type=ReasonSegDataset,
    data_root=reasonseg_data_root + "lisa",
    image_folder=reasonseg_data_root + "lisa/train",
    explain_path=reasonseg_data_root + "lisa/explanatory/train.json",
    tokenizer=tokenizer,
    task_name="reasonseg",
    data_name="lisa_reasonseg_train",
    cond_type=cond_type,
    special_tokens=special_tokens,
    extra_image_processor=train_extra_image_processor,
    image_processor=image_processor,
    ...
    pad_image_to_square=False,
    ignore_label=ignore_label,
    repeats_scale=1.0,  # 根据实际数据量调整
)

# Combine training datasets: geochat, genseg, ovseg, refseg, reasonseg
...
val_datasets = [
    ...
    # 5. Reasoning Segmentation (reasonseg) - LISA validation data
    dict(
        type=ReasonSegDataset,
        data_root=reasonseg_data_root + "lisa",
...
val_evaluators = [
    ...
    # 5. Reasoning Segmentation (reasonseg) - LISA validation
    dict(
...
custom_hooks = [
    ...
    dict(
        ...
        postprocess_fns=[
            ...
            reason_seg_postprocess_fn,  # reasonseg
        ],
        extra_image_processor=extra_image_processor,
