    ...
    for batch_idx, cur_input_ids in enumerate(input_ids):
        ...
        if num_images == 0 and num_regions == 0:
            ...
            if isinstance(pixel_values, list) and len(pixel_values) > 0:
                empty_pixel_values = pixel_values[0][0:0]
            elif isinstance(pixel_values, torch.Tensor) and pixel_values.numel() > 0:
                empty_pixel_values = pixel_values[0][0:0]
            else:
                # 如果 pixel_values 为空，创建一个空张量
                device = cur_inputs_embeds.device
                dtype = cur_inputs_embeds.dtype
                empty_pixel_values = torch.empty((0, cur_inputs_embeds.shape[-1]), device=device, dtype=dtype)
            cur_inputs_embeds = torch.cat([cur_inputs_embeds, empty_pixel_values], dim=0)
        ...
        for i in range(len(all_special_token_indices) - 1):
            ...
            if i < len(all_special_token_indices) - 1 and all_special_token_indices[i + 1][1] != "none":
                ...
                if token_type == "image":
                    # 使用 batch_idx 来索引 pixel_values，因为每个样本对应一个 pixel_values
                    # 如果 pixel_values 是列表，直接使用 batch_idx；如果是张量，也使用 batch_idx
