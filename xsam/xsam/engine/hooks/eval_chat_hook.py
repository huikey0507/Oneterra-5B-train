...
class EvaluateChatHook(Hook):
    ...
    def __init__(
        ...
        if self.evaluation_images is not None:
            # Debug: print actual values before assertion
            eval_images_len = len(self.evaluation_images)
            eval_inputs_len = len(self.evaluation_inputs)
            if eval_images_len not in [1, eval_inputs_len]:
                print_log(
                    f"ERROR: evaluation_images length mismatch!\n"
                    f"  evaluation_images length: {eval_images_len}\n"
                    f"  evaluation_inputs length: {eval_inputs_len}\n"
                    f"  evaluation_images: {self.evaluation_images}\n"
                    f"  evaluation_inputs (first 2): {self.evaluation_inputs[:2] if len(self.evaluation_inputs) >= 2 else self.evaluation_inputs}",
                    logger="current",
                )
            assert eval_images_len in [1, eval_inputs_len], (
                f"evaluation_images length ({eval_images_len}) must be 1 or "
                f"equal to evaluation_inputs length ({eval_inputs_len}). "
                f"evaluation_images: {self.evaluation_images}, "
                f"evaluation_inputs length: {eval_inputs_len}"
            )
