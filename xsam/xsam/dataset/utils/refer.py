...
class REFER:
    ...
    def _load_data(self, splitBy):
        ...
        if not osp.isfile(ref_file):
            raise FileNotFoundError(
                f"REFER refs file not found: {ref_file}. "
                f"Ensure DATA_DIR={self.DATA_DIR} contains refs({splitBy}).p (or use refs(train).p and matching splitBy)."
            )
        if not osp.isfile(instances_file):
            raise FileNotFoundError(
                f"REFER instances file not found: {instances_file}. "
                f"Ensure DATA_DIR={self.DATA_DIR} contains instances.json."
            )
