...
def decode_mask(segmentation, height, width):
    ...
    if isinstance(segmentation, list) and len(segmentation) == 0:
        raise ValueError("decode_mask: segmentation is an empty list")
    if isinstance(segmentation, dict):
        # COCO RLE: "counts" 可以是字符串（压缩）或 list（未压缩 run-length），pycocotools.decode 均支持
        # 不要对 list 调用 .decode("utf-8")，否则会 AttributeError（remotesam 多为字符串，FAST 等可能有 list）
