#!/usr/bin/env bash
# 4xA40 launch for B2 + SARDET mixed training.
#
# Config + code both live in the main repo (git-tracked):
#   xsam/xsam/configs/.../s_sar_align/xsam_sar_B2_cond_adapter_pano_sardet_4xA40.py
#   xsam/xsam/dataset/sar_ov_seg_dataset.py, model/modules/sar_cond_adapter.py, ...
#
# WORK_DIR outputs stay under wkdrs_* (not for GitHub).
#
# Usage (single node 4 GPU):
#   bash runs/run_B2_sardet_4xA40.sh
#
# Optional env overrides:
#   CUDA_VISIBLE_DEVICES=0,1,2,3
#   WORK_DIR=... PREV_B2_CKPT=... MASTER_PORT=29501
#   CODE_DIR=...  # default: $ROOT/xsam

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
CODE_DIR="${CODE_DIR:-${ROOT_DIR}/xsam}"
CONFIG_ABS="${ROOT_DIR}/xsam/xsam/configs/xsam/phi3_mini_4k_instruct_siglip2_so400m_p14_384/s_sar_align/xsam_sar_B2_cond_adapter_pano_sardet_4xA40.py"

WORK_DIR="${WORK_DIR:-${ROOT_DIR}/wkdrs_sar_align_B2_cond_v3_sardet}"
DATA_DIR="${DATA_DIR:-${ROOT_DIR}/datas}"
INIT_DIR="${INIT_DIR:-${ROOT_DIR}/inits}"
PREV_B2_CKPT="${PREV_B2_CKPT:-${ROOT_DIR}/wkdrs_sar_align_B2_cond_v2/pytorch_model.bin}"
...
if [[ ! -f "${PREV_B2_CKPT}" ]]; then
  echo "B2 checkpoint not found: ${PREV_B2_CKPT}" >&2
  exit 1
fi
...
echo "[INFO] PREV_B2=${PREV_B2_CKPT}"
